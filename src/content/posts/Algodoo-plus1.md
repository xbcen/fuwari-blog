...
2. 函数平移提高/降低移动速度请同时扩大/缩小 `x₁,y₁,sim.time` 的值。  
如：`vel = [1 * a, 3 * a * (sim.time * a) ^ 2 + 0.1633334]`  
3. 若需绘制圆锥曲线，请直接使用 $\left\{\begin{aligned}x&=sin x \\y&=cos x \end{aligned}\right.$ 形式的参数方程。
4. 为避免运动轨迹不理想，请避免在三角函数中出现小数。
...
注意事项：  
1. 截取的是以原点为中心，`2 * 2n`的长方形内的部分。  
2. 若将`x₁, y₁`分别扩大`a, b`倍，则会在以物件为中心，`2a * 2bn`的长方形内运动。
